...
test_server:
	g++ -o test src/tests/unit_tests_server.cpp src/server_actions.cpp $(COMMON_FILES) src/logger.cpp -lpthread -lstdc++fs -std=c++17 $(COMPILER_FLAGS) -lpqxx -lpq
test_client:
	g++ -o test src/tests/unit_tests_client.cpp src/client_actions.cpp $(COMMON_FILES) -lstdc++fs -std=c++17 $(COMPILER_FLAGS) -lpqxx -lpq
